        ...
        if results.exists():
            for part in results:
                if part.qty > 0:
                    ...
                    messages.warning(request, "Parts quantity cannot go below zero.")
...
def ml_view(request):
    ...
    for part in parts:
        try:
            ...
            # Skip non-numeric values in qty field
            continue
    ...
    for i in range(len(X_test)):
        predicted_qty = model.predict([X_test[i]])[0]
        part_name = label_encoder.inverse_transform([y_test[i]])[0]
        predictions.append({'part_name': part_name, 'predicted_qty': predicted_qty})

    # Pass the results to the template for rendering
    context = {
        'predictions': predictions,
    }
    return render(request, 'ml_template.html', context)
